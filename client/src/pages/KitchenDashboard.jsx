  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold leading-tight tracking-tight">🍳 <span className="text-primary-400">Kitchen</span> Panel</h1>
          <p className="text-surface-400 mt-2 text-sm">Manage orders and active pools</p>
        </div>
        <button onClick={tab === 'orders' ? fetchOrders : fetchPools} className="btn-secondary flex items-center gap-2 text-sm min-h-[44px] px-4 py-2.5 self-start" id="refresh-kitchen">
          <HiOutlineRefresh className="w-4 h-4" /> Refresh
        </button>
      </div>

      {/* Tabs */}
      <div className="flex gap-2 sm:gap-3 mb-6 md:mb-8 border-b border-surface-700/50 pb-4">
        <button
          onClick={() => setTab('orders')}
          className={`px-4 sm:px-5 py-2.5 rounded-xl text-sm font-medium transition-all min-h-[44px] ${tab === 'orders' ? 'tab-active' : 'bg-surface-800/40 text-surface-400 hover:bg-surface-700/40 border border-surface-700/30'}`}
        >
          📋 Live Orders
        </button>
        <button
          onClick={() => setTab('pools')}
          className={`px-4 sm:px-5 py-2.5 rounded-xl text-sm font-medium transition-all min-h-[44px] ${tab === 'pools' ? 'tab-active' : 'bg-surface-800/40 text-surface-400 hover:bg-surface-700/40 border border-surface-700/30'}`}
        >
          🏊‍♂️ Active Pools
        </button>
      </div>
      ...
      {tab === 'orders' ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6 md:mb-8">
            {[
              ...
              <div key={s.label} className="glass-card-static p-4 md:p-5 text-center">
                ...
                <p className="text-[10px] text-surface-500 uppercase tracking-wider mt-1">{s.label}</p>
              </div>
            ))}
          </div>
          ...
          {loadingOrders ? (
            ...
            <div className="space-y-3 md:space-y-4">
              {orders.map(order => {
                ...
                return (
                  <div key={order._id} className="glass-card-static p-4 md:p-6" id={`kitchen-order-${order._id}`}>
                    ...
                    {/* Order Items */}
                    <div className="bg-surface-900/50 rounded-xl p-3 md:p-4 mb-4">
                      ...
                      {order.specialInstructions && (
                        <p className="text-xs text-amber-400 mt-3 pt-3 border-t border-surface-700/50">
                          📝 {order.specialInstructions}
                        </p>
                      )}
                    </div>
                    ...
                    {cfg.next && (
                      <button
                        ...
                        className={`${cfg.next === 'ready' ? 'btn-success' : 'btn-primary'} w-full text-sm min-h-[44px]`}
                        id={`action-${order._id}`}
                      >
                        {cfg.next === 'preparing' && '👨‍🍳 '}
                        {cfg.next === 'ready' && '✅ '}
        ...
        <>
          ...
          {loadingPools ? (
            ...
            <div className="grid gap-4 md:gap-6 sm:grid-cols-2">
              {pools.map(pool => (
                <div key={pool._id} className="glass-card-static p-4 md:p-6 relative overflow-hidden flex flex-col justify-between">
                    ...
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="badge badge-warning text-[10px] shrink-0">{pool.status}</span>
                        </div>
                        <p className="text-sm text-surface-400 mt-0.5">#{pool._id.slice(-6).toUpperCase()}</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      ...
                      <div className="p-3 rounded-xl bg-surface-900/50 border border-surface-700/30 flex items-center justify-between">
                        <div className="flex items-center gap-2 text-surface-400">
                          ...
                          <span className="text-xs uppercase font-semibold tracking-wider">Closes</span>
                        </div>
                  ...
                  <button
                    ...
                    className="btn-danger w-full text-sm mt-auto min-h-[44px]"
                  >
                    Force Close & Push to Queue
                  </button>
                </div>
              ))}
            </div>
